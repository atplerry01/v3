...
public sealed class ChainRecoveryEngine : IIntelligenceEngine<ChainRecoveryCommand, ChainRecoveryResult>
{
    public string EngineName => "ChainRecovery";

    public IntelligenceResult<ChainRecoveryResult> Execute(IntelligenceContext<ChainRecoveryCommand> context)
    {
        var startedAt = DateTimeOffset.UtcNow;
        var command = context.Input;
        var result = ExecuteCore(command);
        return IntelligenceResult<ChainRecoveryResult>.Ok(result, IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    }

    private ChainRecoveryResult ExecuteCore(ChainRecoveryCommand command)
    {
