...
using ChainAuditResult = Whycespace.Engines.T3I.Reporting.Chain.Models.ChainAuditResult;
using ChainAuditCommand = Whycespace.Engines.T3I.Reporting.Chain.Models.ChainAuditCommand;

public sealed class ChainAuditEngine : IIntelligenceEngine<ChainAuditCommand, ChainAuditResult>
{
    public string EngineName => "ChainAudit";

    public IntelligenceResult<ChainAuditResult> Execute(IntelligenceContext<ChainAuditCommand> context)
    {
        var startedAt = DateTimeOffset.UtcNow;
        var command = context.Input;
        var result = ExecuteCore(command);
        return IntelligenceResult<ChainAuditResult>.Ok(result, IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    }

    private static ChainAuditResult ExecuteCore(ChainAuditCommand command)
    {
