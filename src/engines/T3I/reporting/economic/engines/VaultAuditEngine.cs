...
public sealed class VaultAuditEngine : IIntelligenceEngine<VaultAuditInput, VaultAuditResult>
{
    public string EngineName => "VaultAudit";

    public IntelligenceResult<VaultAuditResult> Execute(IntelligenceContext<VaultAuditInput> context)
    {
        var startedAt = DateTimeOffset.UtcNow;
        var input = context.Input;
        var result = ExecuteAudit(input.Command, input.LedgerEntries, input.Transactions);
        return IntelligenceResult<VaultAuditResult>.Ok(result, IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    }
