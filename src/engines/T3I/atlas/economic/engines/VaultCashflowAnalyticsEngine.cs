...
public sealed class VaultCashflowAnalyticsEngine : IEngine, IIntelligenceEngine<EngineContext, EngineResult>
{
    public string Name => "VaultCashflowAnalytics";
    public string EngineName => "VaultCashflowAnalytics";
    ...
    public IntelligenceResult<EngineResult> Execute(IntelligenceContext<EngineContext> context)
    {
        ...
        if (command is null)
            return IntelligenceResult<EngineResult>.Fail("Invalid cashflow analytics command: missing required fields",
        ...
        if (command.AnalysisEndTimestamp <= command.AnalysisStartTimestamp)
            return IntelligenceResult<EngineResult>.Fail("AnalysisEndTimestamp must be after AnalysisStartTimestamp",
                IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        if (!Enum.TryParse<CashflowAnalysisScope>(command.AnalysisScope, true, out _))
            return IntelligenceResult<EngineResult>.Fail($"Invalid AnalysisScope: {command.AnalysisScope}",
                IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        var ledgerEntries = ResolveLedgerEntries(engineContext);
