...
public sealed class CapitalBalanceEngine : IIntelligenceEngine<CapitalBalanceInput, CapitalBalanceResult>
{
    ...
    public string EngineName => "CapitalBalance";

    public IntelligenceResult<CapitalBalanceResult> Execute(IntelligenceContext<CapitalBalanceInput> context)
    {
        var startedAt = DateTimeOffset.UtcNow;
        var command = context.Input.Command;
        var capitalRecords = context.Input.CapitalRecords;

        var validationError = Validate(command);
        if (validationError is not null)
        {
            var failResult = CapitalBalanceResult.Fail(validationError);
            return IntelligenceResult<CapitalBalanceResult>.Fail(validationError,
        ...
        var result = CapitalBalanceResult.Ok(snapshot);
        return IntelligenceResult<CapitalBalanceResult>.Ok(result,
            IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    }
