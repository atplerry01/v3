namespace Whycespace.Engines.T3I.Atlas.Analytics.Engines;
...
public sealed class AnalyticsEngine : IEngine, IIntelligenceEngine<EngineContext, EngineResult>
{
    public string Name => "Analytics";
    public string EngineName => "Analytics";
    ...
    public IntelligenceResult<EngineResult> Execute(IntelligenceContext<EngineContext> context)
    {
        ...
        var analysisType = engineContext.Data.GetValueOrDefault("analysisType") as string;
        if (string.IsNullOrEmpty(analysisType))
            return IntelligenceResult<EngineResult>.Fail("Missing analysisType",
                IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        var engineResult = analysisType switch
        {
            "ClusterPerformance" => AnalyzeClusterPerformance(engineContext),
            "SpvHealth" => AnalyzeSpvHealth(engineContext),
            "RevenueBreakdown" => AnalyzeRevenueBreakdown(engineContext),
            "WorkflowEfficiency" => AnalyzeWorkflowEfficiency(engineContext),
            "DriverUtilization" => AnalyzeDriverUtilization(engineContext),
            "PropertyOccupancy" => AnalyzePropertyOccupancy(engineContext),
            _ => EngineResult.Fail($"Unknown analysisType: {analysisType}")
        };

        if (!engineResult.Success)
            return IntelligenceResult<EngineResult>.Fail(
                engineResult.Output.GetValueOrDefault("error") as string ?? "Unknown error",
                IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    ...
    private static EngineResult AnalyzeClusterPerformance(EngineContext context)
    {
        var clusterId = context.Data.GetValueOrDefault("clusterId") as string;
        if (string.IsNullOrEmpty(clusterId))
            return EngineResult.Fail("Missing clusterId");
    ...
    private static EngineResult AnalyzeSpvHealth(EngineContext context)
    {
        var spvId = context.Data.GetValueOrDefault("spvId") as string;
        if (string.IsNullOrEmpty(spvId))
            return EngineResult.Fail("Missing spvId");
    ...
    private static EngineResult AnalyzeRevenueBreakdown(EngineContext context)
    {
        var spvId = context.Data.GetValueOrDefault("spvId") as string;
        if (string.IsNullOrEmpty(spvId))
            return EngineResult.Fail("Missing spvId");
    ...
    private static EngineResult AnalyzeWorkflowEfficiency(EngineContext context)
    {
    ...
    private static EngineResult AnalyzeDriverUtilization(EngineContext context)
    {
    ...
    private static EngineResult AnalyzePropertyOccupancy(EngineContext context)
    {
