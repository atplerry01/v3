...
public sealed class ForecastEngine : IEngine, IIntelligenceEngine<EngineContext, EngineResult>
{
    public string Name => "Forecast";
    public string EngineName => "Forecast";
    ...
    public IntelligenceResult<EngineResult> Execute(IntelligenceContext<EngineContext> context)
    {
        ...
        var forecastType = engineContext.Data.GetValueOrDefault("forecastType") as string;
        if (string.IsNullOrEmpty(forecastType))
            return IntelligenceResult<EngineResult>.Fail("Missing forecastType",
                IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        var engineResult = forecastType switch
        {
            "TaxiDemand" => ForecastTaxiDemand(engineContext),
            "PropertyDemand" => ForecastPropertyDemand(engineContext),
            "RevenueForecast" => ForecastRevenue(engineContext),
            "CapacityPlanning" => ForecastCapacity(engineContext),
            _ => EngineResult.Fail($"Unknown forecastType: {forecastType}")
        };
    ...
    private static EngineResult ForecastTaxiDemand(EngineContext context)
    {
    ...
    private static EngineResult ForecastPropertyDemand(EngineContext context)
    {
    ...
    private static EngineResult ForecastRevenue(EngineContext context)
    {
    ...
    private static EngineResult ForecastCapacity(EngineContext context)
    {
