namespace Whycespace.Engines.T3I.Monitoring.Observability.Engines;
...
public sealed class ObservabilityEngine : IEngine, IIntelligenceEngine<EngineContext, EngineResult>
{
    public string Name => "Observability";
    public string EngineName => "Observability";
    ...
    public IntelligenceResult<EngineResult> Execute(IntelligenceContext<EngineContext> context)
    {
        ...
        var eventType = engineContext.Data.GetValueOrDefault("eventType") as string;
        if (string.IsNullOrEmpty(eventType))
            return IntelligenceResult<EngineResult>.Fail("Missing eventType", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        var engineResult = eventType switch
        {
            "EngineInvocation" => ProcessEngineInvocation(engineContext),
            "WorkflowExecution" => ProcessWorkflowExecution(engineContext),
            "SystemHealth" => ProcessSystemHealth(engineContext),
            "ErrorRate" => ProcessErrorRate(engineContext),
            _ => ProcessGenericMetric(eventType, engineContext)
        };
    ...
    private static EngineResult ProcessEngineInvocation(EngineContext context)
    {
    ...
    private static EngineResult ProcessWorkflowExecution(EngineContext context)
    {
    ...
    private static EngineResult ProcessSystemHealth(EngineContext context)
    {
    ...
    private static EngineResult ProcessErrorRate(EngineContext context)
    {
        var component = context.Data.GetValueOrDefault("component") as string ?? "unknown";
    ...
    private static EngineResult ProcessGenericMetric(string eventType, EngineContext context)
    {
        var events = new[]
        {
