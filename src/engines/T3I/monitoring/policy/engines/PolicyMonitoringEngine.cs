...
using Whycespace.Engines.T3I.Monitoring.Policy.Models;
using Whycespace.Engines.T3I.Shared;
namespace Whycespace.Engines.T3I.Monitoring.Policy.Engines;

using Whycespace.Systems.Upstream.WhycePolicy.Models;

public sealed class PolicyMonitoringEngine : IIntelligenceEngine<PolicyMonitoringInput, PolicyMonitoringReport>
{
    ...
    public string EngineName => "PolicyMonitoring";

    public IntelligenceResult<PolicyMonitoringReport> Execute(IntelligenceContext<PolicyMonitoringInput> context)
    {
        var startedAt = DateTimeOffset.UtcNow;
        var input = context.Input;
        var result = AnalyzeMonitoringWindow(input);
        return IntelligenceResult<PolicyMonitoringReport>.Ok(result, IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    }
