...
namespace Whycespace.Engines.T3I.Monitoring.Chain.Engines;

public sealed class ChainHealthMonitorEngine : IIntelligenceEngine<ChainHealthMonitorCommand, ChainHealthMonitorResult>
{
    private const long ReplicationLagDegradedThreshold = 5;
    private const long AnchorLagDegradedThreshold = 10;

    public string EngineName => "ChainHealthMonitor";

    public IntelligenceResult<ChainHealthMonitorResult> Execute(IntelligenceContext<ChainHealthMonitorCommand> context)
    {
        var startedAt = DateTimeOffset.UtcNow;
        var command = context.Input;
        var result = ExecuteCore(command);
        return IntelligenceResult<ChainHealthMonitorResult>.Ok(result, IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    }

    public ChainHealthMonitorResult ExecuteCore(ChainHealthMonitorCommand command)
    {
