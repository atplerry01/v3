...
public sealed class VaultFraudDetectionEngine : IEngine, IIntelligenceEngine<EngineContext, EngineResult>
{
    private const double FraudAlertThreshold = 60.0;

    public string Name => "VaultFraudDetection";
    public string EngineName => "VaultFraudDetection";
    ...
    public IntelligenceResult<EngineResult> Execute(IntelligenceContext<EngineContext> context)
    {
        ...
        if (!engineContext.Data.TryGetValue("vaultId", out var vaultIdObj) ||
            !Guid.TryParse(vaultIdObj?.ToString(), out var vaultId))
            return IntelligenceResult<EngineResult>.Fail("Missing or invalid vaultId", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        if (!engineContext.Data.TryGetValue("transactionId", out var txIdObj) ||
            !Guid.TryParse(txIdObj?.ToString(), out var transactionId))
            return IntelligenceResult<EngineResult>.Fail("Missing or invalid transactionId", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        if (!engineContext.Data.TryGetValue("vaultAccountId", out var acctIdObj) ||
            !Guid.TryParse(acctIdObj?.ToString(), out _))
            return IntelligenceResult<EngineResult>.Fail("Missing or invalid vaultAccountId", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        if (!engineContext.Data.TryGetValue("initiatorIdentityId", out var identIdObj) ||
            !Guid.TryParse(identIdObj?.ToString(), out _))
            return IntelligenceResult<EngineResult>.Fail("Missing or invalid initiatorIdentityId", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        var amount = ResolveDecimal(engineContext.Data.GetValueOrDefault("amount"));
        if (amount is null || amount < 0)
            return IntelligenceResult<EngineResult>.Fail("Missing or invalid amount", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        var operationType = engineContext.Data.GetValueOrDefault("operationType")?.ToString();
        if (string.IsNullOrWhiteSpace(operationType))
            return IntelligenceResult<EngineResult>.Fail("Missing operationType", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        var currency = engineContext.Data.GetValueOrDefault("currency")?.ToString();
        if (string.IsNullOrWhiteSpace(currency))
            return IntelligenceResult<EngineResult>.Fail("Missing currency", IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));

        // --- Resolve behavioral context (optional inputs for fraud analysis) ---
        var recentTransactionCount = ResolveInt(engineContext.Data.GetValueOrDefault("recentTransactionCount")) ?? 0;
        var recentTransactionWindowMinutes = ResolveInt(engineContext.Data.GetValueOrDefault("recentTransactionWindowMinutes")) ?? 60;
        var averageTransactionAmount = ResolveDecimal(engineContext.Data.GetValueOrDefault("averageTransactionAmount")) ?? 0m;
        var recentFailedOperationCount = ResolveInt(engineContext.Data.GetValueOrDefault("recentFailedOperationCount")) ?? 0;
        var recentLargeWithdrawalCount = ResolveInt(engineContext.Data.GetValueOrDefault("recentLargeWithdrawalCount")) ?? 0;
        var accountAgeDays = ResolveInt(engineContext.Data.GetValueOrDefault("accountAgeDays")) ?? 365;
        var isNewIdentity = ResolveBool(engineContext.Data.GetValueOrDefault("isNewIdentity"));
        ...
        return IntelligenceResult<EngineResult>.Ok(EngineResult.Ok(events, output), IntelligenceTrace.Create(EngineName, context.CorrelationId, startedAt));
    }
