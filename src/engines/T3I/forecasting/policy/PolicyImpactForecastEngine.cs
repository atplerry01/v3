...
{
    public PolicyImpactForecastResult ForecastImpact(PolicyImpactForecastInput input)
    {
        var records = new List<PolicyImpactRecord>();
        var affectedPolicies = new HashSet<string>();

        foreach (var context in input.SimulationContexts)
        {
            var currentDecision = EvaluatePolicies(input.CurrentPolicies, context);
            var proposedDecision = EvaluatePolicies(input.ProposedPolicies, context);
            ...
            if (impactType != ImpactType.NO_CHANGE)
            {
                ...
                {
                    if (MatchesContext(policy, context))
        ...
        return new PolicyImpactForecastResult(
            orderedRecords,
            sortedAffected,
            riskLevel,
            DateTime.UtcNow
