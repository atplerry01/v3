...
public sealed class ChainHealthMonitorEngine
{
    ...
    private static string VerifyBlockContinuity(IReadOnlyList<System.Upstream.WhyceChain.Models.ChainBlock> blocks)
    {
        if (blocks.Count <= 1)
            return "Valid";

        var sorted = new List<System.Upstream.WhyceChain.Models.ChainBlock>(blocks);
    ...
    private static string VerifyLedgerIntegrity(IReadOnlyList<System.Upstream.WhyceChain.Models.ChainLedgerEntry> entries)
    {
        if (entries.Count <= 1)
            return "Valid";
